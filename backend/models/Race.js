...
class Race {
  ...
  static async create(pool, { nom_race, prix_achat, prix_vente, prix_vente_male, prix_oeuf, prix_nourriture, male, femelle, nb_jours_eclosion, capacite_ponte, oeufs_pourris, deces_male, deces_femelle }) {
    const result = await pool
      .request()
      ...
      .input('prix_vente_male', sql.Decimal(10, 2), prix_vente_male ?? null)
      ...
      .query(
        'INSERT INTO race (nom_race, prix_achat, prix_vente, prix_vente_male, prix_oeuf, prix_nourriture, male, femelle, nb_jours_eclosion, capacite_ponte, oeufs_pourris, deces_male, deces_femelle) OUTPUT INSERTED.* VALUES (@nom_race, @prix_achat, @prix_vente, @prix_vente_male, @prix_oeuf, @prix_nourriture, @male, @femelle, @nb_jours_eclosion, @capacite_ponte, @oeufs_pourris, @deces_male, @deces_femelle)'
      );
    return result.recordset[0];
  }

  static async update(pool, id, { nom_race, prix_achat, prix_vente, prix_vente_male, prix_oeuf, prix_nourriture, male, femelle, nb_jours_eclosion, capacite_ponte, oeufs_pourris, deces_male, deces_femelle }) {
    const result = await pool
      .request()
      .input('id', sql.Int, id)
      .input('nom_race', sql.VarChar(255), nom_race ?? null)
